...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the DEFAULT_USER username if it exists
    if id DEFAULT_USER &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER DEFAULT_USER
    fi
    ...
    if [[ "$NB_USER" != "DEFAULT_USER" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/DEFAULT_USER "/home/$NB_USER"
        fi
        # if workdir is in /home/DEFAULT_USER, cd to /home/$NB_USER
    ...
    if [[ "$NB_UID" == "$(id -u DEFAULT_USER)" && "$NB_GID" == "$(id -g DEFAULT_USER)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^DEFAULT_USER:/nayvoj:/" > /tmp/passwd
                echo "DEFAULT_USER:x:$(id -u):$(id -g):,,,:/home/DEFAULT_USER:/bin/bash" >> /tmp/passwd
